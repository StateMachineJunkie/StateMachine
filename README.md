...
At one point, I went to work for Traveling Software,the creators of LapLink™. The Laplink app allowed users to perform file transfers and run remote-control sessions between two PC computers running Windows 95™, ideally one of them being a laptop computer. Physical connections between the computers were facilitated using the standard IBM-PC serial-port, parallel-port or, alternatively, the IPX LAN networking protocol.

During my tenure at TS, I became responsible for implementing the serial-port and parallel-port protocols, and hardware support for the new Windows version of this product (Previously a DOS-only implementation was available but it was before my time, for the most part.)
...
For my implementation, I needed something that was thread-safe and could easily be specified, configured and re-configured, audited at run-time for correctness, and did not rely on dynamic memory allocation since virtual DOS mode was a thing in Windows 95. There were other constraints but the memory is fading. In any case, I decided to re-create my state machine implementation as a possible component for a personal communications project I'm building for the iPad but in Swift and using Swift Concurrency, where appropriate. I wanted my solution to be compatible with Swift 6 with Strick Concurrency. This project is the result of my researc thus far.
